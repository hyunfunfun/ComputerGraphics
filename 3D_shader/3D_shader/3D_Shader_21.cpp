...
GLvoid drawScene() {
	...
	unsigned int viewLocation1 = glGetUniformLocation(shaderProgramID, "viewTransform"); //--- 뷰잉 변환 설정
	unsigned int projectionLocation1 = glGetUniformLocation(shaderProgramID, "projectionTransform"); //--- 투영 변환 값 설정

	unsigned int viewLocation2 = glGetUniformLocation(shaderProgramID, "viewTransform"); //--- 뷰잉 변환 설정
	unsigned int projectionLocation2 = glGetUniformLocation(shaderProgramID, "projectionTransform"); //--- 투영 변환 값 설정
	...
	projection1 = glm::ortho(-5.0f, 5.0f, -5.0f, 5.0f, -5.0f, 5.0f);
	...
	view = glm::lookAt(cameraPos2, cameraDirection1, cameraUp1);
	glUniformMatrix4fv(viewLocation, 1, GL_FALSE, &view[0][0]);
...
GLvoid keyboard(unsigned char key, int x, int y) {
	...
	case 'r':
		...
		cameraPos1.z = dis * cos(yAngle);
		cameraPos1.x = dis * sin(yAngle);
	...
	case 'R':
		...
		cameraPos1.z = dis * cos(yAngle);
		cameraPos1.x = dis * sin(yAngle);
		cameraPos2.z = dis * cos(yAngle);
		cameraPos2.x = dis * sin(yAngle);
		break;
	case 'a':
